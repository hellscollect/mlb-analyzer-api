...
def _fetch_statcast_csv(mbam: int, start_dt: str, end_dt: str, *, timeout: float=8.0, max_retries:int=2, retry_backoff:float=0.6) -> List[Dict[str,str]]:
    params = {
        "player_type":"batter","player_lookup":"true","type":"details",
        "player": str(mbam), "start_date": start_dt, "end_date": end_dt,
...
def _calc_signal(rows: Iterable[Dict[str,str]]) -> Tuple[Optional[float], Optional[float]]:
    evs: List[float]=[]; xbas: List[float]=[]; events: List[str]=[]
    for row in rows:
        ls=row.get("launch_speed")
        if ls:
            try: evs.append(float(ls))
            except Exception: pass
        xba=row.get("estimated_ba_using_speedangle")
        if xba:
            try: xbas.append(float(xba))
            except Exception: pass
        events.append((row.get("events") or "").strip().lower())
    hard_hit_pct=None
    if evs:
        hard=sum(1 for ev in evs if ev>=95.0)
        hard_hit_pct=100.0*hard/len(evs)
    AB_EVENTS={"single","double","triple","home_run","strikeout","field_out","grounded_into_double_play","force_out","field_error","double_play","fielders_choice","fielders_choice_out","pop_out","flyout","lineout","strikeout_double_play","other_out"}
    HIT_EVENTS={"single","double","triple","home_run"}
    ab=sum(1 for e in events if e in AB_EVENTS); hits=sum(1 for e in events if e in HIT_EVENTS)
...
def _meets_thresholds(hard_hit_pct: Optional[float], xba_gap: Optional[float], *, hh_min: float, xba_delta_min: float)->Tuple[bool,str]:
    reasons=[]; ok=False
    if hard_hit_pct is not None and hard_hit_pct>=hh_min:
        ok=True; reasons.append(f"HH% {hard_hit_pct:.1f}≥{hh_min:.0f}")
    if xba_gap is not None and xba_gap>=xba_delta_min:
        ok=True; reasons.append(f"xBA–BA +{xba_gap:.3f}≥+{xba_delta_min:.3f}")
    return ok, "; ".join(reasons)

def fetch_statcast_overlays(names: List[str], *, recent_days:int=STATCAST_DAYS_DEFAULT, statcast_min_hh_14d:float=40.0, statcast_min_xba_delta_14d:float=0.030)->Dict[str,Dict[str,Any]]:
    ...
    for nm in names:
        try:
            ...
            result={"has_signal":bool(has),"why":why,"hh_percent_14d":round(hh,1) if hh is not None else None,"xba_delta_14d":round(xgap,3) if xgap is not None else None}
            _to_cache(ck,result); out[nm]=result
            time.sleep(0.08)
        except Exception:
