...
def _mlb_schedule(date_obj: date_cls) -> List[Dict[str, Any]]:
    """Fetch MLB schedule for date from StatsAPI and normalize to ET fields."""
...
def league_scan_post(req: LeagueScanReq, request: Request):
    ...
    def run_for(d: date_cls) -> Dict[str, Any]:
        ...
        return {
            ...
            "matchups": upcoming,  # has home/away, et_time, venue, probables
            "debug": {
    ...
    out_today = run_for(primary_date)
    if out_today["counts"]["matchups"] >= 1:
        return out_today

    out_tmr = run_for(tomorrow)
